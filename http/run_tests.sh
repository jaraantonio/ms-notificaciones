#!/usr/bin/env bash
# ============================================================================
# run_tests.sh — Ejecuta todos los requests de ms-notificaciones.http
# ============================================================================
...
HTTP_FILE="$SCRIPT_DIR/ms-notificaciones.http"
...
run_request() {
    ...
    # Construir curl
    local curl_args=(-s -w "%{http_code}" -o /tmp/ms_notif_resp.txt -X "$method")
    curl_args+=("$clean_url")
    ...
    local response_body
    response_body=$(cat /tmp/ms_notif_resp.txt 2>/dev/null || echo "")
...
echo -e "${BOLD}╔══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║  Perfulandia SPA — Test ms-notificaciones (HTTP runner)║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""

# Health check
if ! curl -s --connect-timeout 3 -o /dev/null "${BASE_URL}/actuator/health"; then
    echo -e "${RED}ERROR:${NC} ${BASE_URL} no responde. Iniciá ms-notificaciones."
    exit 1
fi
