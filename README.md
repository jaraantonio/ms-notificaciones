...
- Java 25, Spring Boot 4.0.6, Spring Web MVC, Spring Mail, JPA/Hibernate
...
La aplicación usa MySQL driver (compatible con MariaDB vía XAMPP). La base de datos `perfulandia_notificaciones` se crea automáticamente al arrancar (`createDatabaseIfNotExist=true`). Las tablas se crean vía Hibernate (`ddl-auto=update`). Los registros de prueba se insertan con `data.sql` al iniciar (`spring.sql.init.mode=always`).
